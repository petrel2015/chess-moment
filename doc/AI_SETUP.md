...
本网站的"问教练"功能有三档，按配置程度从低到高：

1. **免 Key 免费中转（零配置，默认）**：什么都不配，浏览器直连一个第三方免费
   转发（Pollinations 匿名层）拿真 AI 回答。**实验性**——第三方免费转发无 SLA，
   可能停用/限流/中文质量一般；失败时自动退回预制答案，不会卡住用户。
2. **预制答案（确定性兜底）**：每道题预先写好的解答。免费中转不可用时的兜底。
3. **智谱 GLM-4-Flash（推荐，稳定）**：配一个 Cloudflare Worker 代理，走智谱官方
   免费模型。需要约 5–10 分钟、两次免费注册，之后访客零配置即用真 AI。

按本指南接入第 3 档，可以拿到最稳定、中文质量最好的真 AI 回答。
